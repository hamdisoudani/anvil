...
export function AgentThinking({ events, defaultExpanded = false, className, compact = false, }) {
    ...
    return (_jsxs("div", { className: containerClass, children: [_jsxs("button", { type: "button", onClick: () => setExpanded(!expanded), className: "w-full flex items-center gap-3 text-left", children: [_jsx("div", { className: cn("flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-full shrink-0", isActive ? "bg-primary/10 text-primary" : state.phase === "error" ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"), children: _jsx(Icon, { className: cn("h-3 sm:h-3.5 w-3 sm:w-3.5", isActive && "animate-spin") }) }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("div", { className: "text-xs sm:text-sm font-medium truncate", children: activity }), _jsxs("div", { className: "text-[10px] sm:text-xs text-muted-foreground", children: [PHASE_LABELS[state.phase], state.plan && state.plan.steps.length > 0 && (_jsxs("span", { children: [" \u00B7 ", state.plan.steps.length, " steps"] })), state.sources.length > 0 && (_jsxs("span", { children: [" \u00B7 ", state.sources.length, " sources"] })), state.pagesRead > 0 && (_jsxs("span", { children: [" \u00B7 ", state.pagesRead, " pages read"] }))] })] }), _jsx(ChevronRight, { className: cn("h-3.5 w-3.5 text-muted-foreground transition-transform shrink-0", expanded && "rotate-90") })] }), expanded && (_jsxs("div", { className: "space-y-3 pt-2 border-t", children: [state.plan && (_jsx("div", { className: "space-y-1.5", children: state.plan.steps.length > 0 && (_jsx("div", { className: "flex flex-wrap gap-1", children: state.plan.steps.map((s, i) => (_jsx(Badge, { variant: "secondary", className: "text-[9px] sm:text-[10px]", children: s.detail || s.intent }, s.id || i))) })) })), state.sources.length > 0 && (_jsxs("div", { className: "space-y-1.5", children: [_jsxs("button", { type: "button", onClick: () => setShowSources(!showSources), className: "flex items-center gap-1.5 text-[10px] text-muted-foreground hover:text-foreground", children: [_jsx(ChevronRight, { className: cn("h-3 w-3 transition-transform", showSources && "rotate-90") }), "Sources (", state.sources.length, ")"] }), showSources && (_jsx("div", { className: "grid grid-cols-1 gap-1", children: state.sources.map((s) => (_jsxs("a", { href: s.url, target: "_blank", rel: "noopener noreferrer", className: "flex items-center gap-2 p-1.5 rounded hover:bg-accent/30 text-[10px] sm:text-xs", children: [_jsx(Badge, { variant: "outline", className: "h-4 px-1 text-[9px] font-mono shrink-0", children: s.id }), _jsx("span", { className: "truncate", children: s.title }), _jsx("span", { className: "text-muted-foreground truncate hidden sm:inline", children: s.domain })] }, s.id))) }))] })), state.planSteps.length > 0 && (_jsxs("div", { className: "space-y-1.5", children: [_jsxs("button", { type: "button", onClick: () => setShowSteps(!showSteps), className: "flex items-center gap-1.5 text-[10px] text-muted-foreground hover:text-foreground", children: [_jsx(ChevronRight, { className: cn("h-3 w-3 transition-transform", showSteps && "rotate-90") }), "Steps (", state.planSteps.length, ")"] }), showSteps && (_jsx("div", { className: "space-y-0.5 pl-2 border-l-2 border-muted", children: state.planSteps.map((step, i) => (_jsxs("div", { className: "flex items-center gap-2 py-0.5 text-[10px] sm:text-xs", children: [_jsx("div", { className: cn("w-1.5 h-1.5 rounded-full shrink-0", step.status === "running" ? "bg-primary animate-pulse" :
                                                step.status === "done" ? "bg-green-500" :
                                                    step.status === "error" ? "bg-destructive" : "bg-muted-foreground") }), _jsxs("span", { className: cn("truncate", step.status === "running" ? "font-medium" : "text-muted-foreground"), children: [step.intent, step.detail ? `: ${step.detail}` : ""] }), step.status === "done" && _jsx(CheckCircle2, { className: "h-3 w-3 text-green-500 shrink-0" })] }, i))) }))] }))] }))] }));
}
...
export function AgentThinkingInline({ events }) {
    ...
    if (state.phase === "idle")
        return null;
    return (_jsxs("div", { className: "flex items-center gap-2 text-xs text-muted-foreground animate-pulse", children: [_jsx(Loader2, { className: "h-3 w-3 animate-spin" }), _jsx("span", { children: getActivity(state) })] }));
}
