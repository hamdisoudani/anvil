...
// First middleware is outermost. Returns step unchanged if none configured.
func (s *Session) applyMiddleware(step MiddlewareStep) MiddlewareStep {
	if len(s.middleware) == 0 {
		return step
	}
	wrapped := step
	// Compose right-to-left so index 0 is outermost.
...
// Middleware is applied around tool execution (see executeTool).
func (s *Session) loop() {
	defer close(s.done)
	for {
		select {
		...
		// 2. Act — if a tool call, execute it (idempotent + middleware)
