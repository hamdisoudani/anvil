...
func (o *Orchestrator) tryFrontendTools(
	...
	systemPrompt := `You are an AI assistant with access to browser-side UI tools. You may call any of them when the user asks you to change the chat UI's appearance, focus, or layout. If the user's request doesn't need a frontend tool, just respond with a short acknowledgement and stop. Do not write a long answer — the main answer will be generated by another step.`
